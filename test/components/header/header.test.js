...
describe('header', () => {
  ...
  describe('custom options', () => {
    ...
    it('renders custom container classes', async () => {
      ...
      ok($container.hasClass('govuk-header__container--full-width'))
    })
  ...
  describe('with service name', () => {
    ...
    it('does not use a link when no service url is provided', async () => {
      const $ = await render('header', examples['with service name but no service url'])
  ...
  describe('with navigation', () => {
    ...
    it('renders navigation label and menu button text when these are both set', async () => {
      const $ = await render('header', examples['with custom navigation label and custom menu button text'])
    ...
    it('renders navigation with active item', async () => {
      ...
      ok($activeItem.hasClass('govuk-header__navigation-item--active'))
    })
    ...
    it('renders navigation item with text without a link', async () => {
      const $ = await render('header', examples['navigation item with text without link'])
    ...
    it('renders navigation item with html without a link', async () => {
      const $ = await render('header', examples['navigation item with html without link'])
  ...
  for (const { message, exampleName } of [
    ...
    describe(message, () => {
      ...
      before(async () => {
        $ = await render('header', examples[exampleName])
      })

      it('does not include a menu button', () => {
        equal($('.govuk-header__menu-button').length, 0)
      })

      it('does not include a <nav> element', () => {
        equal($('nav').length, 0)
      })

      it('does not include a govuk-header__content wrapper', () => {
  ...
  describe('SVG logo', () => {
    ...
    it("uses the St Edward's Crown if useTudorCrown is false", async () => {
      const $ = await render('header', examples["with St Edward's crown"])
      const $svg = $('.govuk-header__logotype')
